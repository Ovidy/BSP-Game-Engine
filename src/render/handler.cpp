...
namespace render {
    ...
    void Handler::render(const Camera3D& raylib_camera, const glm::vec2& camera_position, const std::vector<glm::int32_t>& current_segment_ids) {
        ...
        render_2d(camera_position, current_segment_ids);
        
        // end the frame and get ready for the next one  (display frame, poll input, etc...)
        EndDrawing();
    }

    void Handler::render_2d(const glm::vec2& camera_position, const std::vector<glm::int32_t>& current_segment_ids) {
        map_renderer.render(current_segment_ids, camera_position);
        DrawFPS(10, 10);
    }
