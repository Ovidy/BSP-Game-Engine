...
namespace render {
    ...
    std::vector<Segment> MapRenderer::get_normalized_segments() const {
        ...
        for (const auto& segment : segments) {
            ...
            normalized_segments.push_back({midpoint, midpoint + normal, -1, segment.get_sector()});
        }
    ...
    std::vector<Segment> MapRenderer::remap_segments(const std::vector<Segment>& segments_) const {
        ...
        for (const auto& segment : segments_) {
            remapped_segments.push_back({remap_vec2(segment.get_start()), remap_vec2(segment.get_end()), segment.get_texture_id(), segment.get_sector()});
        }
