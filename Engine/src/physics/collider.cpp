...
namespace engine
{
        ...
        const std::vector<Sector> &level_sectors)
    {
        ...
        for (const auto &sector : level_sectors)
        {
            ...
            if (feet_y >= sector.ceiling_height)
                continue;
            if (head_y <= sector.floor_height)
                continue;

            if (result.total_collision())
                return result;
    ...
    glm::vec2 Collider::closest_point_on_segment(const glm::vec2 &p, const glm::vec2 &a, const glm::vec2 &b)
    {
        ...
        if (dot_ab == 0.0f)
            return a; // Wall is just a single point (shouldn't happen, but safe)
    ...
    bool Collider::check_axis(const glm::vec2 &pos, float player_radius, const Segment &wall)
    {
