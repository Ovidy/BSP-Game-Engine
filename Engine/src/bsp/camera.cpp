...
namespace engine
{
    ...
    void Camera::update(const glm::float32_t &dt, const std::vector<Sector> &level_sectors)
    {
        check_velocity();
    ...
    void Camera::move(const std::vector<Sector> &level_sectors, float dt)
    {
        ...
        if (!noclip_enabled)
        {
            ...
            CollisionResult hit = Collider::detect_wall_collision(
            ...
            VerticalBounds bounds = Collider::get_sector_bounds(get_pos_2d(), feet_y, head_y, level_sectors);
